...
(function(){  // Self Executing Function Used for Encapsulation.
	...
	var search = function(query){ // Fixed Error, Added Opening Curly Brace.
		...
		for(var i=0, j=db.length; i<j; i++) {
            ...
            var dbitem = db[i].toLowerCase()    .substring(0, dbTitleEnd); //fixed camelCase on toLowerCase from tolowercase

            // loop through the user's search query words
            // save a lowercase variable of the search keyword
            for (var ii = 0, jj = queryArray.length; ii < jj; ii++) {
                var qitem = queryArray[ii].toLowerCase(); //fixed camelCase on toLowerCase
            ...
            };// Fixed: Added Curly Brace
        };// Fixed: Added Curly Brace
		
		results.sort();
		
		// Check that matches were found, and run output functions
		if(results.length == 0){   //fixed = to == (to assign not compare)
			noMatch();
		}else{
			showMatches(results);
		};
